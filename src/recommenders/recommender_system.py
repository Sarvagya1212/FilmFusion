...
class RecommenderSystem:
    ...
    def build_content_model(self):
        ...
        for col in self.content_cols:
            if col in df.columns:
                soup_components.append(df[col].astype(str))
        
        if not soup_components:
            raise ValueError("No valid content columns found to build the content model.")
    ...
    def recommend_hybrid(self, user_id, top_n=10, filter_seen=True, alpha=0.8, beta=0.2, delta=0.0):
        user_cf = self.recommend_user_based(user_id=user_id, top_n=len(self.user_item_matrix.columns), filter_seen=False)
        item_cf = self.recommend_item_based(user_id=user_id, top_n=len(self.user_item_matrix.columns), filter_seen=False)
        ...
        if 'compound' in self.metadata_df.columns:
            sentiment_scores = self.metadata_df.set_index('tmdbId')['compound'].to_dict()
            sentiment_scores = normalize(sentiment_scores)
        else:
            sentiment_scores = {}

        all_ids = set(user_cf_scores) | set(item_cf_scores) | set(sentiment_scores)

        hybrid_scores = {
            tid: alpha * user_cf_scores.get(tid, 0)
            + beta * item_cf_scores.get(tid, 0)
            + delta * sentiment_scores.get(tid, 0)
            for tid in all_ids
        }
